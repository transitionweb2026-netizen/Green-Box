...
import { updateOurStoryContentAction } from "@/app/admin/(dashboard)/content/actions";
import type { SettingsActionState } from "@/app/admin/(dashboard)/content/actions";
import type { OurStoryContent } from "@/lib/services/content";

interface Field {
  key: keyof OurStoryContent & string;
  labelAr: string;
  multiline?: boolean;
}
...
export function OurStoryContentForm({ content }: { content: OurStoryContent | null }) {
  const [state, formAction, isPending] = useActionState(updateOurStoryContentAction, { status: "idle" } as SettingsActionState);

  return (
    <form action={formAction} className="space-y-6">
      <p className="text-sm text-muted">محتوى صفحة &quot;قصتنا&quot;. اترك أي حقل فاضي عشان يظهر النص الافتراضي بدل منه.</p>
      ...
      <Button type="submit" size="sm" disabled={isPending} loading={isPending}>
        ...
        {isPending ? "جارٍ الحفظ..." : "حفظ صفحة قصتنا"}
      </Button>
    </form>
  );
}
