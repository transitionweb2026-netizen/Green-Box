-- Connects the existing products.requires_reservation flag to checkout,
-- which previously ignored it entirely (a product could be marked
-- "requires reservation" and still be ordered for same-day delivery with
-- no lead time at all). The actual cutoff/lead-time policy is a real,
-- still-open business decision (see DECISIONS.md Q11) -- rather than
-- inventing one, this makes it a plain admin-editable setting with a
-- conservative 1-day default, stored in the existing generic `settings`
-- table (key 'reservation_settings', shape {"lead_days": <int>}) so no new
-- table is needed and the admin UI reuses the same settings pattern as
-- store_info.

insert into public.settings (key, value, description)
values ('reservation_settings', jsonb_build_object('lead_days', 1), 'Minimum days of advance notice required for products marked "requires reservation".')
